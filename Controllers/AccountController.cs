...
namespace ShoppingCart_Application_MVC.Controllers
{
    ...
    // allow Anonymous is basically user to view perticular part of application
    [AllowAnonymous]
    public class AccountController : Controller
    {
        ...
        public ActionResult LoginUser(LoginUser user)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var registeredUser = db.RegisterUser.FirstOrDefault(model => (model.Email == user.EmailOrPhone || model.Phone == user.EmailOrPhone)
            ...
            {
                ViewBag.Failed = $"An error occurred: {ex.Message}";
            }
        ...
        [HttpPost]
        public ActionResult RegisterUser(RegisterUser reg)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    var checkEmail = db.RegisterUser.Count(model => model.Email == reg.Email);
                    var checkPhone = db.RegisterUser.Count(model => model.Phone == reg.Phone);

                    if (checkEmail > 0)
                    {
                        ViewBag.emailExist = "Email is already registered!";
                        return View();
                    }

                    if (checkPhone > 0)
                    {
                        ViewBag.PhoneExist = "Phone Number is already registered!";
                        return View();
                    }

                    string passwordHash = HashPassword(reg.Password);
